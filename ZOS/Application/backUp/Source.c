...
const uint8_t FAT_UNUSED = UINT8_MAX - 0;
const uint8_t FAT_FILE_END = UINT8_MAX - 1;
const uint8_t FAT_BAD_CLUSTER = UINT8_MAX - 2;
const uint8_t FAT_DIRECTORY = UINT8_MAX - 3;

#define CLUSTER_SIZE 256			//size of cluster
#define USABLE_CLUSTERS 251			//number of usable clusters
#define NODES_PER_CLUSTER 10		//possible nodes pre cluster
#define CLEAR_SIGN '\0'				//clearing char
...
/*
checks if cluster could be understand as data cluster (no directory or unused space or out of range)
	return:	true	for possible data cluster
			false	not data cluster
*/
bool is_valid_data_cluster(int cluster) {
	if (cluster > UINT8_MAX || cluster < 0) return false;
	if (fat[cluster] == FAT_UNUSED || fat[cluster] == FAT_DIRECTORY) return false;
	return true;
}

/*
checks if one of the fat tables have valid directory cluster reference if primary table fails and secondary succeeds
primary table is corrected by secodary
	cluster:	examined cluster
*/
bool validate_directory_cluster(int cluster) {
	if (cluster > UINT8_MAX || cluster < 0) return false;
	if (fat[cluster] == FAT_DIRECTORY) return true;
	if (fat_sec[cluster] == FAT_DIRECTORY) {
		fat[cluster] = fat_sec[cluster];
		printf(" directory cluster corrected from secondary FAT table\n");
		return true;
	}
	return false;
}
...
clear_rest_clusters_file(int index) {
	...
	while (fat[cl] != FAT_FILE_END && is_valid_data_cluster(cl)) {
		cl_old = cl;
		cl = fat[cl];
		set_unused(cl_old);
	}
...
sets min_free_cluster appropriately
	fat_index:	cluster of index to be set as unused
*/
...
fills spaces where atributes name from nodes should be with CLEAR_SIGN characters
...
create_directory(char name[], char target[]) {
	...
	//duplicity check
	if (!name_duplicity_check(name, dir_position, offset)) {
		printf("directory already contains record with name %s\n", name);
		name_correction(node_p);
	}
	printf("OK\n");
}
...
list_file_node(struct node *node_p, int depth) {
	...
	if (!is_valid_data_cluster(cl)) {	//file with invalid adress will not be listed
		return;
	}
...
list_dir_node(struct node *node_p, int depth) {
	...
	if (node_p->start_cluster > UINT8_MAX || node_p->start_cluster < 0) {
		return;		//invalid directory will not be listed
	}
...
name_correction(struct node *node_p) {
	if (strlen(node_p->name)>12) {
		printf(" name (%.20s) was too long and was shortened to standard (12 characters)\n", node_p->name);
		node_p->name[12] = '\0';
		node_p->name[8] = '.';
	}
	...
	int node_offset = (node_adr - cls_adr) / (sizeof(*node_p));	//node offset
	if (name_duplicity_check(node_p->name, cl_index, node_offset) != true) {
		printf(" name change caused by duplicity %s\n", node_p->name);
		//duplicity found
		int index = 0;

		if (strlen(node_p->name) <= 1) {
			//short name
			do {
			...
			//normal name
			char name_n[13];
			memset(name_n, '\0', sizeof(node_p->name));	//clear
			name_n[0] = '(';
			name_n[1] = '#';
			name_n[2] = ')';
			for (int i = 3; i < 13; i++) {
				name_n[i] = node_p->name[i - 3];	//copy from third
			}
			strcpy(node_p->name, name_n);	//copy
			do {
				node_p->name[1] = ('0' + index);
				index++;
			} while (name_duplicity_check(node_p->name, cl_index, node_offset) != true);
			printf("  new name: %s\n", node_p->name);
		}
...
mark_directory(struct node *dir, int *fat_cnt) {
	...
	int cl = dir->start_cluster;
	if (!validate_directory_cluster(cl)) {
		printf(" invalid directory cluster address\n");
		clear_node(dir);	//remove
	...
	if (fat_cnt[cl] > 1) {
		...
		clear_node(dir);	//remove
		printf(" multiple ways to same directory at cluster %d (%d hits)\n", dir->start_cluster, fat_cnt[dir->start_cluster]);
		printf("  unnecessary reference was removed\n");
		//stop recursive branching
		return;
	}

	//recursive iterate trough it's directories
	struct node *node_p = &clusters[cl];
	for (int i = 0; i < NODES_PER_CLUSTER; i++) {
		if (is_free_node(node_p)) {
			//skip empty node record
			node_p++;
...
mark_file(struct node *file, int *fat_cnt) {
	...
	//possible correction with secondary fat table
	fat_correction_file(file);
	fat_cnt[cl]++;	//start file
	...
	while (fat[cl] != FAT_FILE_END) {
		...
		if (!is_valid_data_cluster(cl) || fat_cnt[cl] >= 1) {
		...
		if (file_cnt > exp_cnt) {
			...
			printf(" corrupted file %s expected number of used clusters was %d (file size: %dB)\n", file->name, exp_cnt, file->size);
			fat[cl_old] = FAT_FILE_END;	//end file early
			fat_sec[cl_old] = FAT_FILE_END;
			clear_rest_clusters_file(cl);
			return;
...
prevents cycles by use of hit test marking and ends the files one step before cycle
...
add_to_list(int val, int *list, int *list_cnt, int *fat_cnt) {
	...
	while (true) {
		...
		if (fat_cnt[fat[cl]] >= 1 || !is_valid_data_cluster(fat[cl])) {
			//points to something what was already hit
			fat[cl] = FAT_FILE_END;
			fat_sec[cl] = FAT_FILE_END;
			break;
		}

		if (fat[cl] == FAT_FILE_END) {				
			break;									//add to list
		}
		for (int i = 0; i < *list_cnt; i++) {		//look trough list of found ones
			if (fat[cl] == list[i]) {
				list[i] = val;
				return;								//value in list replaced
			}
		}
...
file_recovery(int *fat_cnt) {
	...
	for (int i = 0; i < USABLE_CLUSTERS; i++) {
		if (is_valid_data_cluster(i) && fat_cnt[i] == 0) {
	...
	for (int i = 0; i < list_cnt; i++) {
		...
		if (offset == -1) {
			...
			printf("  unrecovered files were deleted and marked as unused \n");
			for (int j = i; j < list_cnt; i++) {
				clear_rest_clusters_file(list[i]);		//deletes rest of the files
			}
			free(list);
			return;
		}
...
/*
Checks file sizes using both tables if one of them is correct (size and FAT_FILE_END match the file) 
primary table is corrected by the second one otherwise there is no effect
serves as protection again one table damage
*/
fat_correction_file(struct node *file) {
	int cl_f = file->start_cluster;
	int cl_s = cl_f;
	int size = file->size;
	int exp_cls = (size / CLUSTER_SIZE) + 1;
	int file_cls = 0;

	if (size < 0 || size > 251 * 256) return;	//incorrect size of file

	//iterate both tables
	while (file_cls < exp_cls) {
		cl_f = fat[cl_f];
		cl_s = fat_sec[cl_s];
		file_cls++;
	}
	...
	if (cl_f == FAT_FILE_END) {
		//primary table
		return;
	}
	if (cl_s == FAT_FILE_END) {
		//repair primary from secondary
		printf(" corrections from secondary FAT table\n");
		cl_f = file->start_cluster;
		file_cls = 0;
		while (file_cls < exp_cls) {
			fat[cl_f] = fat_sec[cl_f];
			cl_f = fat_sec[cl_f];
			file_cls++;
		}
		return;
...
dump_structure(char *name, struct boot_record *br) {
	...
	fwrite(br, sizeof(*br), 1, file);
	fwrite(fat, sizeof(uint8_t), USABLE_CLUSTERS, file);
	fwrite(fat_sec, sizeof(uint8_t), USABLE_CLUSTERS, file);
	fwrite(clusters, sizeof(clusters[0]), USABLE_CLUSTERS, file);
...
main(int argc, char *argv[]) {
	...
	if (swt[0] == 'a' && argc == 5) {
		...
		create_directory(argv[3], argv[4]);
	}

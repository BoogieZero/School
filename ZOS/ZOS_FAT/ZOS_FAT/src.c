...
as there is no valid cluster available
	return:	valid index
			-1 for no available valid index
*/
...
mark_file(struct node *file, int *fat_cnt) {
	...
	while (fat[cl] != FAT_FILE_END) {
		...
		if (fat[cl] == FAT_UNUSED) {
			//unused space that could belong to file
...
main(int argc, char *argv[]) {
	...
	if (swt[0] == 'a' && argc == 5) {
		...
		//dump_structure("test_corrupted.dat", br_p);
		list_structure();
		check_structure();
		list_structure();
		//check_structure();

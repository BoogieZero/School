...
static frequencies *freqs_head;	
	
/* pointer to first transmitter in list of all transmitters */
static transmitters *trans_head;
	
/* pointer to top of stack */
static transmitters *m_stack;
...
int get_block_number(char line[]){
	if(strncmp(line, s_available, sizeof s_available-1) == 0){
   		return 1;
	}
	if(strncmp(line, s_radius, sizeof s_radius-1) == 0){
		return 2;
	}
	if(strncmp(line, s_transmitters, sizeof s_transmitters-1) == 0){
		return 3;
	}
	return 0;
}

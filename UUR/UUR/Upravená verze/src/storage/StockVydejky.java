...
public class StockVydejky extends Stock {
	...
	public void removeVydejka(){
		...
		if(alert.getResult()==ButtonType.YES){
			...
			if(!checkEveryItem(vy)){
				Main.Main.safe.zbozi.wrongInput(Main.Main.safe.zbozi.ER_33);
				return;
			}
	...
	/**
	 * Zkontroluje zda lze vrátit všechno zboží na sklad
	 * @param v	zadaná výdejka
	 * @return	true pokud lze zboží vrátit
	 */
	private boolean checkEveryItem(Vydejka v){
		...
		for(items.Zbozi sZb: list){
			...
			}else{	//if zbozi was deleted before
				return false;
			}
		}
		return true;
	...
	private void returnZboziByVydejka(Vydejka v){
		...
		for(items.Zbozi sZb: list){
			...
			if(puvZb!=null){
				...
				System.out.println("Nektery typ zbozi byl záměrně ostraněn ze záznamů");
			}
